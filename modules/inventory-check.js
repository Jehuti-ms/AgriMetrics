    ...
    this.element.innerHTML = `
        <div class="module-container">
            ...
            <div class="stats-grid">
                <div class="stat-card">
                    <div>📦</div>
                    <div id="total-items">${stats.totalItems}</div>
                    <div>Total Items</div>
                </div>
                <div class="stat-card">
                    <div>✅</div>
                    <div id="in-stock">${stats.inStock}</div>
                    <div>In Stock</div>
                </div>
                <div class="stat-card">
                    <div>⚠️</div>
                    <div id="low-stock" style="color:${lowStockItems.length > 0 ? '#f59e0b' : '#22c55e'};">
                ...
                <div class="stat-card">
                    <div>💰</div>
                    <div id="total-value">${this.formatCurrency(stats.totalValue)}</div>
                    <div>Total Value</div>
                </div>
            </div>
            ...
            <div class="quick-action-grid">
                ...
                <button class="quick-action-btn" id="low-stock-report-btn">
                    ...
                    <span>View items to reorder</span>
                </button>
                <button class="quick-action-btn" id="inventory-report-btn">
                    <div>📈</div>
                    <span>Inventory Report</span>
                    <span>Full inventory analysis</span>
                </button>
            </div>
            ...
            <div id="stock-update-container" class="hidden">
                <div class="glass-card">
                    <h3 id="stock-update-title">Update Stock Level</h3>
            ...
            ${lowStockItems.length > 0 ? `
                <div class="glass-card" style="border-left:4px solid #f59e0b;">
                    <div class="card-header">
                        ...
                        <span class="badge">${lowStockItems.length} item${lowStockItems.length > 1 ? 's' : ''}</span>
                    </div>
                    <div class="stats-grid">
                        ${lowStockItems.map(item => `
                            <div class="stat-card">
                                <div>${item.name}</div>
                                <div>${item.currentStock} ${item.unit} • Min: ${item.minStock} ${item.unit}</div>
                                <button class="btn-outline restock-item" data-id="${item.id}">Restock Now</button>
                            </div>
                        `).join('')}
                    </div>
                </div>
                ...
                <div class="card-header">
                    ...
                    <div class="header-actions">
                        <select id="category-filter">
                            ...
                            ${this.categories.map(cat => `<option value="${cat}">${this.formatCategory(cat)}</option>`).join('')}
                        </select>
