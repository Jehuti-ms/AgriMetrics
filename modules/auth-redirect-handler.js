...
class AuthRedirectHandler {
  ...
  handleRedirect() {
    ...
    // ✅ Skip redirect handler unless a redirect flow was attempted
    if (!sessionStorage.getItem('authMethod')) {
      console.log('⚡ Skipping redirect handler (no redirect authMethod)');
      return;
    }
    ...
    // Otherwise, check for redirect result (Google/Apple flows)
